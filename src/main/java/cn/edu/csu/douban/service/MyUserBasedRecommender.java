...
import cn.edu.csu.douban.model.MyDataModel;
...
public class MyUserBasedRecommender {
	private static int NEIGHBORHOOD_SIZE = 10;
	...
	public List<RecommendedItem> userBasedRecommender(long userID,int size) {
		...
		try {
			...
			UserNeighborhood neighborhood = new NearestNUserNeighborhood(NEIGHBORHOOD_SIZE, similarity, model);//计算用户的“邻居”，这里将与该用户最近距离为 NEIGHBORHOOD_SIZE 的用户设置为该用户的“邻居”。
