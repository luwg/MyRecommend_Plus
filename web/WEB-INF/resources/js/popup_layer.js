...
var PopupLayer = new Class({
	...
	_init:function(options){
		...
		this.popupLayer = this.popupLayer?$(this.popupLayer):$(document.createElement("div")).addClass(this.options.popupLayerClass);
		this.popupLayer.attr('id', 'popupLayer');
		...
		var _this = this;
		this.trigger.mouseover(this.options.eventType,function(){            //给触发元素绑定触发事件
			...
			$.get(
				...
				function(movie) {
					...
					str += "<h2>" + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + "</h2>";
					str += "<h2>评分：<span style='color: red;'>" + movie.rate + "</span></h2>"
					str += "<h2>" + movie.lengthOfFilm + "分钟</h2>";
					str += "<h2>" + movie.language + "</h2>";
					str += "<h2>" + movie.category + "</h2></div>";
					_this.popupBlk.html(str);
				}
			)
			if(_this.isSetPosition){
				_this.setPosition($(this).offset().left + _this.options.offsets.x, $(this).offset().top + this.offsetHeight + _this.options.offsets.y);
			}
			_this.options.useOverlay?_this._loadOverlay():null;               //如果使用遮罩则加载遮罩元素
			(_this.isOverlay && _this.options.useOverlay)?_this.overlay.show():null;
			if(_this.isDoPopup && (_this.popupLayer.css("display")== "none")){
				_this.options.useFx?_this.doEffects("open"):_this.popupLayer.show();
			}
		});
